...
BEGIN
    ...
    -- Sprawdï¿½, czy data rezerwacji jest pï¿½niejsza niï¿½ rozpoczï¿½cie repertuaru
    IF :NEW.data_rezerwacji > v_data_rozpoczecia THEN
        RAISE_APPLICATION_ERROR(-20001, 'Nie moï¿½na rezerwowaï¿½ biletï¿½w na seans, ktï¿½ry juï¿½ siï¿½ zaczï¿½ï¿½.');
    END IF;
END;
/
...
BEGIN
    -- ZnajdÅº ID istniejÄ…cej rezerwacji
    ...
    -- SprawdÅº czy miejsca zostaÅ‚y zwolnione
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Rezerwacja ' || v_rezerwacja_id || ' zostaÅ‚a anulowana.');
END;
/
