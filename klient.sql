...
CREATE OR REPLACE PACKAGE BODY Klient_Pkg AS
    ...
    BEGIN
        ...
        IF miejsce_do_zarezerwowania < ilosc_miejsc_do_zarezerwowania THEN
            RAISE_APPLICATION_ERROR(-20009, 'Nie ma wystarczajacej liczby wolnych miejsc w wybranym rz�dzie.');
        END IF;
...
BEGIN
    ...
    FOR r IN (
        ...
        -- Pobranie miejsc zaj�tych w tej rezerwacji
        FOR b IN (
            ...
            DBMS_OUTPUT.PUT_LINE(' -> Zaj�te miejsce: Rz�d ' || b.rzad || ', Miejsce ' || b.miejsce);
        END LOOP;
    END LOOP;
    DBMS_OUTPUT.PUT_LINE('------------------------------------------');
