...
BEGIN
 ...
 DECLARE
  v_rola VARCHAR2(20);
 BEGIN
  SELECT rola INTO v_rola FROM Uzytkownik_table WHERE email = 'jan@test.pl';
...
BEGIN
 ...
 BEGIN
  Admin_Pkg.dodaj_seans(id_filmu => 2, id_sali => 2, data_rozpoczecia_filmu => TRUNC(SYSDATE - 2) + (10/24));
  DBMS_OUTPUT.PUT_LINE('Seans filmu "Kraina Lodu" dodany pomyslnie.');
  Klient_Pkg.Zarezerwuj_Seans(email_uzytkownika => 'jan@test.pl', tytul_filmu => 'Kraina Lodu', data_seansu_in => TRUNC(SYSDATE - 2) + (10/24), preferencja_rzedu => 2, ilosc_miejsc_do_zarezerwowania => 5);
  Admin_Pkg.dodaj_seans(id_filmu => 2, id_sali => 1, data_rozpoczecia_filmu => TRUNC(SYSDATE + 5) + (10/24));
  DBMS_OUTPUT.PUT_LINE('Seans filmu "Kraina Lodu" dodany pomyslnie.');
  Klient_Pkg.Zarezerwuj_Seans(email_uzytkownika => 'jan@test.pl', tytul_filmu => 'Kraina Lodu', data_seansu_in => TRUNC(SYSDATE + 5) + (10/24), preferencja_rzedu => 3, ilosc_miejsc_do_zarezerwowania => 1);
  Klient_Pkg.Zarezerwuj_Seans(email_uzytkownika => 'zbigniew@test.pl', tytul_filmu => 'Kraina Lodu', data_seansu_in => TRUNC(SYSDATE + 5) + (10/24), preferencja_rzedu => 5, ilosc_miejsc_do_zarezerwowania => 3);
